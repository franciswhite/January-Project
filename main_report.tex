...
$m_{stone}$ being played inflicts a cost of $\alpha$ to $P_1$, while $m_{tree}$ bears cost $\beta$ for $P_2$, where $a>\alpha,\beta>0$.\footnote{Communicating is always preferable to non-communication and no expressions can only be harmful.} Using the same ordering as above, the cost vectors are $c^1=(0,\alpha,0,0)$ and $c^2=(0,0,0,\beta)$.
...
% TODO: Use this section elsewhere or delete. Perhaps we use it when talking about expected utility
...
\end{equation*}
We see that $R_1=R_0$, i.e. the receiver did not change strategy from 0-level to 1-level, thus we conclude that $S_2=S_1$. But $S_1$ did thus we calculate:
\begin{equation*}
