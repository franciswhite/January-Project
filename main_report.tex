...
In linguistics, it was applied in attempts to obtain a firmer conceptual grip on Gricean pragmatics.
%References!
...
We are going to use an IBR model to provide our solution concept. %WHY? %What is a solution concept?
...
Communication is a messy affair. Expressions change their meaning over time and each person as well as the words they use carry a history with them. Examples abound where language users harm their interlocutors not by the content of their speech - by revealing some inconvenient truth, for example -, but by choosing inappropriate terms to express the matter. Think of any derogatory name for social groups and compare it to using a more appropriate synonym.%This still needs some work and juiciness.

We will now use the apparatus lately introduced to analyse one scenario of information transmission where certain expressions have the potential to harm the interlocutors.
...
$m_{stone}$ being played inflicts a cost of $\alpha$ to $P_1$, while $m_{tree}$ bears cost $\beta$ for $P_2$, where $a>\alpha,\beta>0$.\footnote{Communicating is always preferable to non-communication and no expressions can only be harmful.} The two cost vectors are thus
...
What is the best response of the level-1 hearer and speaker? Following the IBR procedure we transpose $R_0$, subtract the cost vector $c^1$ and check row-wise for the best response to obtain $S_1$. We proceed analogously for the receiver, the main difference being in this case that the receiver faces some actual cost, $c^2$. This then yields strategies for level-1 players:\\ %TODO : we need to add the prior. %
...
$$c^{1*}=c^{1} + d \cdot c^{2}$$
Where $d \in \mathbb{R}$. The best response is defined as previously.
We can now see that by setting $d=0$ we get the original best response since then $c^{1*}=c^{1}$.
Let us now go through the game again with the extended sender's cost. $S_0$ and $R_0$ remain unchanged because these types don't take cost into account at all, so let us start with $S_1$:
\begin{equation*}
S_1= BR_S(R_0^T-c^{1*})=
BR_S(
\bordermatrix{
