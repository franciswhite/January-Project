...
We model language use as a signaling games. These are sequential games between two players: a sender and a receiver \cite{david1969convention}. The game is player in a particular state of the world, formally an element of set $T$ which represents possibles ways the world could be. Only the sender knows which state this happens to be and would like to transfer this information to the receiver. The receiver, who cannot observe the true state, tries to infer the true state by observing a signal which the sender emits. Moreover, the receiver has some probabilistic beliefs about the possible state which are modeled by a probability distribution over the elements of $T$.
%Do we even make us of the probabilistic beliefs over states?
...
%References! all the papers we read for the week of class
...
As in standard signaling games, $T$ is the set of states the players can be in, $M$ the set of messages at the sender's disposal. We assume that $T$ and $M$ are finite sets, $|M|=m$, $|T|=n$. A sender strategy is a row-stochastic $(n \times m)$-matrix $S$, and a receiver strategy is a row-stochastic $(m\times t)$-matrix $R$.  way of example, here are two strategies for a game with $|S|=|A|=3$ and $|M|=2$, so three states and actions, and two messages.
\textbf{If space needed this example plus its explanation will have to go.}
\begin{equation*}
\sigma =
    \bordermatrix{
          & m_1 & m_2    \cr
...
The best response of a k-level players is based on the interlocutor's k-1 level with message cost taken into account. %Explain better%
Thus we get the best response of the level-1 hearer and speaker: %  Following the IBR procedure we transpose $R_0$, subtract the cost vector $c^1$ and check row-wise for the best response to obtain $S_1$. \tb{No need to spell this out procedurally. Explain intuitively what higher level players do, and then just go straight to the formula. The step-by-step derivations are cumbersome and can be confusing. You have not said, in all generally, how they are computed up to here. So it's hard to digest for a na\"ive reader}  We proceed analogously for the receiver, the main difference being in this case that the receiver faces some actual cost, $c^2$. This then yields strategies for level-1 players:\\ %TODO : we need to add the prior. %
...
% We might want to add some text here and deal with Thomas' comment below.
%tb{What does pareto optimality mean? Related back to your notion of ``better communication'' in the introduction. Pareto optimality captures a combination of behaviors where each agent fares best given the behavior of another agent -- this intuitively captures ... in communication}
...
%We can just skip the formula altogether. I think we should
