...
\end{equation*}
Assume, for example, the sender fully considers the receiver's cost - $d=1$\footnote{Notice, however, that we get the same prediction for any $d>0$.}
\begin{equation*}
S_1=BR_S(
\bordermatrix{
                & & & &    \cr
...
\bordermatrix{
 ...
 =S_*
 \end{equation*}
$R_1=R_0$ thus $S_2=S_1$ and we end with $S_2=S^*$ and $R_2=R^*$ as stable strategies.
As already shown, this strategy pair is Pareto optimal.
\section{Discussion and Conclusion}
Why do peple choose harmful words when there are non-harmful options available?
Analysis in an IBR model has shown that rational language users fail to behave optimally because they fail to consider the other's interest. We argued that they could move to Pareto Optimality if they were to show their interlocutors even a minimal amount of consideration. To make this case, we extended standard IBR with an empathy parameter that measures how the players view the interlocutor's cost.

Due to constraints of space, we could not model that omitting expressions might inflict cost to the sender.
the due to restrictions of space:What's the intuitive take home message? Then add another paragraph explaining shortcomings: When does this fail? What could be made differently? Open questions?
